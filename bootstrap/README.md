...
| Argo CD Application `kube-prometheus-stack` stuck `OutOfSync` with `waiting for completion of hook batch/Job/kps-admission-create`, hook pod is `1/2 NotReady` | Same Istio-sidecar-blocks-Job pattern as Argo CD, but the affected Job is the chart's admission-webhook-patch hook. The `observability` namespace IS mesh-injected, so we can't drop the namespace label this time. | Annotate the patch Jobs to skip injection: in `helm-values/kube-prometheus-stack-values.yaml` under `prometheusOperator.admissionWebhooks.patch.podAnnotations`, set `sidecar.istio.io/inject: "false"`. Already pre-fixed. Recover: `kubectl -n observability delete job kps-admission-create kps-admission-patch 2>/dev/null; argocd app sync kube-prometheus-stack`. |
...
| Gatekeeper rejects pod with `disallowed image registry` | New component's registry not in allowlist. Note: Gatekeeper matches the LITERAL image string in the pod spec — it does not normalize `kiwigrid/foo` to `docker.io/kiwigrid/foo`, so both forms must be allowlisted separately if charts use bare DockerHub references. | Edit `policies/gatekeeper/constraints/allowed-registries-cluster.yaml`, add **both** the bare prefix (`kiwigrid/`) and the fully-qualified one (`docker.io/kiwigrid/`), push, Argo syncs the constraint, then the StatefulSet's controller retries pod creation automatically. |
