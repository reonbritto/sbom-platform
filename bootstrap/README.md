...
| ESO `ClusterSecretStore.READY=False` | Federated credential subject mismatch or KV not reachable | `kubectl -n external-secrets logs -l app.kubernetes.io/name=external-secrets`. Should authenticate via WI; the FIC subject must be `system:serviceaccount:external-secrets:eso-service-account`. |
