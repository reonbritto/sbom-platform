...
| Challenge `Ready=False` with `AADSTS700213: No matching federated identity record found for presented assertion subject 'system:serviceaccount:cert-manager:cert-manager'` | The ExternalDNS MI's federated credential is bound to `external-dns/external-dns-sa`, but cert-manager's pod runs as a different SA. AAD rejects the token swap. | Add a second `azurerm_federated_identity_credential` on `id-sbom-external-dns` with subject `system:serviceaccount:cert-manager:cert-manager`. Already added in `infra/main.tf` — re-apply Terraform. |
